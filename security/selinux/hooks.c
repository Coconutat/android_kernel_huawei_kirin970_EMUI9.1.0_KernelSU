								...
								unsigned long *set_kern_flags)
{
	 ...
	 */
	if ((sbsec->flags & SE_SBINITIALIZED) && (sb->s_type->fs_flags & FS_BINARY_MOUNTDATA) && (num_opts == 0))
		goto out;
...
static int file_map_prot_check(struct file *file, unsigned long prot, int shared)
{
	...
	if (default_noexec &&
		(prot & PROT_EXEC) && (!file || IS_PRIVATE(file_inode(file)) || (!shared && (prot & PROT_WRITE))))
	{
		/*
